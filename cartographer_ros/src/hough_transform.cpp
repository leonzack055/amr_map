...
HoughTransform::HoughTransform(const HoughTransformConfig& config) 
    : config_(config) {
...
std::vector<LineFeature> HoughTransform::DetectLines(
...
std::vector<cv::Vec4i> HoughTransform::RunHoughTransform(const cv::Mat& edge_image) {
  ...
  // 使用OpenCV的概率霍夫变换
  cv::HoughLinesP(edge_image, lines,
                  config_.rho_resolution,
                  config_.theta_resolution,
                  threshold,
                  config_.min_line_length_pixels,
                  config_.max_line_gap_pixels);
  
  return lines;
}

std::vector<LineFeature> HoughTransform::ConvertToLineFeatures(
  ...
  for (const auto& line : lines) {
    ...
    // 转换像素坐标到世界坐标
    feature.start_x = (line[0] * resolution) + origin.x();
    feature.start_y = (line[1] * resolution) + origin.y();
    feature.end_x = (line[2] * resolution) + origin.x();
    feature.end_y = (line[3] * resolution) + origin.y();
...
std::vector<LineFeature> HoughTransform::ValidateLines(
...
double HoughTransform::CalculateConfidence(const LineFeature& line,
                                         const cv::Mat& edge_image) {
  // 计算支持点数
...
int HoughTransform::CalculateEdgeSupport(const LineFeature& line,
                                       const cv::Mat& edge_image) {
  int support_count = 0;
  ...
  while (true) {
    ...
    for (int dy = -tolerance; dy <= tolerance; ++dy) {
      for (int dx = -tolerance; dx <= tolerance; ++dx) {
        ...
        if (check_x >= 0 && check_x < edge_image.cols &&
            check_y >= 0 && check_y < edge_image.rows) {
...
std::vector<LineFeature> HoughTransform::FilterByAngle(
    const std::vector<LineFeature>& lines) {
  // 这里可以根据需要实现角度滤波
  // 目前返回所有线段
  return lines;
...
std::vector<LineFeature> HoughTransform::FilterByLength(
    const std::vector<LineFeature>& lines) {
...
std::vector<LineFeature> HoughTransform::RemoveOverlappingLines(
    const std::vector<LineFeature>& lines) {
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    for (size_t j = i + 1; j < lines.size(); ++j) {
      ...
      if (AreLinesOverlapping(current, lines[j],
                             config_.max_angle_diff,
                             config_.max_distance_diff)) {
...
bool HoughTransform::AreLinesOverlapping(const LineFeature& line1,
...
LineFeature HoughTransform::MergeLines(const LineFeature& line1,
                                      const LineFeature& line2) {
  // 计算合并后的端点
