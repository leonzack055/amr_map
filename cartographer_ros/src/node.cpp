...
{
  // change SensorDataQoS to ParametersQoS
...
: node_options_(node_options)
{
  ...
  map_builder_bridge_.reset(
    new cartographer_ros::MapBuilderBridge(
      node_options_, std::move(map_builder), tf_buffer.get()));
...
# endif
  ...
  get_trajectory_states_server_ =
    ...
    std::bind(
      &Node::handleGetTrajectoryStates, this,
      std::placeholders::_1, std::placeholders::_2));
  read_metrics_server_ =
    node_->create_service<cartographer_ros_msgs::srv::ReadMetrics>(
    kReadMetricsServiceName,
    std::bind(
      &Node::handleReadMetrics, this, std::placeholders::_1,
      std::placeholders::_2));

  // 发布Trajectory Pose 以PoseStamped的格式
...
void Node::PublishLocalTrajectoryData()
{
  ...
  for (const auto & entry : map_builder_bridge_->GetLocalTrajectoryData()) {
    ...
    // TODO: 这里外推器，要改成EKF吗，进行VIO的优化和imu融合外推？？
      ...
      extrapolator.GetLastPoseTime())
    {
      if (scan_matched_point_cloud_publisher_->get_subscription_count() > 0) {
        ...
        point_cloud.reserve(
          trajectory_data.local_slam_data->range_data_in_local
          .returns.size());
        for (const cartographer::sensor::RangefinderPoint & point :
          trajectory_data.local_slam_data->range_data_in_local.returns)
        {
          point_cloud.push_back(
            cartographer::sensor::ToTimedRangefinderPoint(
              point, 0.f /* time */));
        }
    ...
    // 如果不发布新的点云，允许已发布姿势的时间提前。GetLastExtrapolatedTime上一次外推的时间
    // 如果已经知道一个更新时间的姿势，就使用它的时间。
    // 因为tf 知道如何插值，因此提供更好的信息。
    // 如果支持在publish
    // pose时使用外推，那么就使用外推时间当前系统时间，否则laser_scan的获取时间
    // 注:
    // 这里now()也可能小于GetLastExtrapolatedTime；因为其直接使用kExtrapolationEstimationTimeSec
    // =0.01s 进行外推，而当前系统时间可能没到这个外推时间；

    const ::cartographer::common::Time now =
      std::max(FromRos(node_->now()), extrapolator.GetLastExtrapolatedTime());

    // TODO:
    // 对于pose_extrapolator，这里是否需要考虑时间回拨问题？它理应不超过当前系统时间
    // local_slam_data->time
    // 如果远小于now，则给出警告，说明当前计算性能无法满足时间要求；
    // GetLastExtrapolatedTime()
    // 如果大于当前系统时间，说明时间前拔了，会影响其他传感器在tf树
    // 中的时间同步，所有其后发布时间都有警告；这里要考虑odom里程计的时间问题；
    // 不使用外推算法进行过滤，其只发布laser_scan的时间，根据频率再进行选择。
    ...
    // 如果发布时间和上一次发布时间相同，就Suppress
    // publishing；因为tf2会对同时间戳的重复消息发出警告。
    // 由于这里发布的频率超快几乎5s就触发一次发布流程,所以可以进行continue。
    ...
    // 获取tracking_to_map的坐标，一般有imu为/imu_frame，没有则使用odom的/base_link
    ...
    if (trajectory_data.published_to_tracking != nullptr) {
      if (node_options_.publish_to_tf) {
        if (trajectory_data.trajectory_options.provide_odom_frame) {
          // provide_odom_fame lua node配置项，开启时表示没有odom传感器传入
          // 使用激光里程计模式，这里虚拟一个odom出去，使用local_to_map来表示里程计
          // 这样并不好，里程计坐标会跳动。使用odom_frame
          ...
          // TODO: 这里会造成坐标冗余，所以对于一般有里程计的情况，
          // 考虑发布map->odom之间的变换，以保证TF 树解析时的歧义
          // 获取最近可用的变换（timeout设为0直接获取最新）
          if (trajectory_data.trajectory_options.use_odometry) {
            geometry_msgs::msg::TransformStamped odom_transformStamp =
              tf_buffer_->lookupTransform(
              "odom", tracking_frame_,
              tf2::TimePointZero);
            Rigid3d publish_to_odom = ToRigid3d(odom_transformStamp);
            stamped_transform.header.frame_id = node_options_.map_frame;
            stamped_transform.child_frame_id = "odom";
            auto odom_to_map = tracking_to_map * publish_to_odom.inverse();
      ...
      if (node_options_.publish_tracked_pose) {
        // 如果能拿到extroplator中的odom信息，还可以使用NavOdometry进行发布，而不是单纯的posestamp
    ...
    // TODO： 监听里程计回调消息发布
        if(last_publish_stamps_.count(entry.first) == 0)
        {
          last_publish_stamps_[entry.first] = node_->now();
        } else {
          auto current_time = node_->now();
          rclcpp::Duration interval = current_time - last_publish_stamps_[entry.first];
          // 输出时间间隔（毫秒级）
      ...
      RCLCPP_WARN(node_->get_logger(), 
                      "Trajectory%d 没有获取到local_pose",entry.first);
    }
  }
}
...
int Node::AddTrajectory(const TrajectoryOptions & options)
{
  ...
  tracking_frame_ = options.tracking_frame;
...
void Node::FinishAllTrajectories()
{
  ...
  for (const auto & entry : map_builder_bridge_->GetTrajectoryStates()) {
    if (entry.second == TrajectoryState::ACTIVE) {
      ...
      CHECK_EQ(
        FinishTrajectoryUnderLock(trajectory_id).code,
        cartographer_ros_msgs::msg::StatusCode::OK);
    }
  }
}
...
{
  auto current_time = node_->now();
  if(last_receive_laserscan_stamps_.count(sensor_id) == 0)
  {
    last_receive_laserscan_stamps_[sensor_id] = node_->now();
  } else {
    rclcpp::Duration interval = current_time - last_receive_laserscan_stamps_[sensor_id];
    // 输出时间间隔（毫秒级）
    int64_t interval_ms = interval.nanoseconds() / 1000000;  // 纳秒→毫秒
  ...
  auto current_end_time = node_->now();
