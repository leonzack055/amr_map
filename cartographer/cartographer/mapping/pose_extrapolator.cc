...
void PoseExtrapolator::AddOdometryData(
  ...
  //   // 打印里程计数据
  // LOG(INFO) << "odometry_data_newest.time: " << odometry_data_newest.time
  //           << " odometry_data_newest.pose: " << odometry_data_newest.pose;
  // // 打印里程计数据
  // LOG(INFO) << "odometry_data_oldest.time: " << odometry_data_oldest.time
  //           << " odometry_data_oldest.pose: " << odometry_data_oldest.pose;
  ...
  // auto angle_axis_delta = transform::RotationQuaternionToAngleAxisVector(rotation);
  // LOG(INFO) << " odometry_pose_delta.pose: " << odometry_pose_delta;
  // LOG(INFO) << "odometry_pose_delta quatrotation : " << rotation;
  // LOG(INFO) << "odometry_pose_delta. axvector  " << angle_axis_delta;
  // LOG(INFO) << "odometry_time_delta: " << odometry_time_delta;
  ...
  // if(anglue_velocity_unitAxis.z() < 0){
  //   anglue_velocity_unitAxis = -anglue_velocity_unitAxis;
  ...
  // } else {
  //   // 断言vector 与 z 轴夹角
  //   CHECK_GT(anglue_velocity_unitAxis.dot(Eigen::Vector3d::UnitZ()), 0.99) << "里程计推算角速度与yaw方向旋转太大";
  //   rotation_angle = common::NormalizeAngleDifference(rotation_angle);
  // }
  // angular_velocity_from_odometry_ = rotation_angle * Eigen::Vector3d::UnitZ();
  // LOG(INFO) << "修正后里程计计算的角速度: angular_velocity_from_odometry_: " << angular_velocity_from_odometry_;

  if (timed_pose_queue_.empty()) {
    return;
  }
  // TODO： 从里程计推测出来的线速度检测，是否存在z轴漂移
