...
std::vector<SubmapId> FindSubmapIdsToTrim(
  ...
  for (const auto& cell : coverage_grid.cells()) {
    ...
    // submaps are kept.
    // 只保留最新的fresh_submaps_count个submap，一个cell可能有多个子图覆盖它，去除那些旧的子图
    ...
    // 保留下来的子图，每个cell的子图数量+1，当作他作为保留轨迹地图的权重； 当一个比较新的子图
    // 能够有更多的cell与其他子图覆盖时，就说明它在有效行驶地方，且权重比较高；
    for (const std::pair<SubmapId, common::Time>& submap : submaps_per_cell) {
      ++submap_to_covered_cells_count[submap.first];
    }
  }
  // 当保留子图被覆盖的权重比较大时，他就与原来的场景相似度就越多，它就越应该被保留
  // 这个阈值设定为min_covered_cells_count； 
  ...
  // 保留下来的子图，与所有子图的差集，就是需要移除的子图； 而且保证保留的子图是子图ID的顺序
  // 存放在队列中； 可以使用差集来获取，要删除的子图。
...
void OverlappingSubmapsTrimmer2D::Trim(Trimmable* pose_graph) {
  ...
  // 2. 进行覆盖，统计交汇子图范围中的cell包含哪些子图；这个构建的交汇子图可能非常巨大
  // 如果原子图很巨大的情况下； 它要基本乘以3,很多栅格要进行统计； 而且它会卡住AddNode()
  // 从而使整体卡顿； 所以它应该在离线时使用。
  const std::map<SubmapId, common::Time> submap_freshness =
  ...
  // 移除旧的子图，这里相对应的子图的约束也被清理了； 这明显会造成信息损失； 会不会造成孤立子图呢？
